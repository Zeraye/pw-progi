...
const ListItem = (props) => {
  ...
  const setScoreHandler = (event) => {
    ...
    if (!scoreValidation(event.target.value) && event.target.value !== "")
      setDangerScore(true);
  ...
  const editHandler = () => {
    if (scoreValidation(score)) props.updateScores(props.name, score);
  };

  const deleteHandler = () => {
    props.deleteScore(props.name);
  };

  return (
    <li className={`list-group-item ${classes.sub}`}>
      <span className={classes.sub__data}>
        {getName(props.name)} p. {getLevel(props.name)} {"->"} {props.score}%
      </span>
